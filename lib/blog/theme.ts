...
> = {
  // Spec §1.2/§1.3 — exact values from the v2 mock.
  ...
  // Intentional values from the About pillars handoff spec. No live blog post uses
  // these categories yet, so updating them is inert on the blog TODAY; the first
  // optimize/protect/legacy post will render with these (matching the About pillars
  // by design). `dot` tracks each new gradFrom.
